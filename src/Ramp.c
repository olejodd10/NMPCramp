...
void ramp_init(size_t n_H, size_t n_a, void (*get_column_M4)(size_t, real_t*)) {
    ...
    indexed_vectors_init(&m_invq_coefs, 2*n_a, n_H, n_H);
...
static int compute_v(size_t n_H, const ordered_set_t* a_set, const indexed_vectors_t *invq_coefs, size_t index, real_t q0, real_t v_coefs[n_H]) {
    ...
    v_coefs[index] = 0.0;
    for (size_t n = 0, i = ordered_set_nth(a_set, n); i != ordered_set_end(a_set); i = ordered_set_nth(a_set, ++n)) {
        lincomb_add_scaled(n_H, a_set, v_coefs, indexed_vectors_get(invq_coefs, i), column_M4[i], v_coefs);
    }
    // On inserts the new index will not be inserted yet, so we need to compensate manually
    real_t vi = lincomb_element(n_H, a_set, v_coefs, &m_m4_cols, index) + column_M4[index];
    real_t qdiv = q0 + vi;
    ...
    lincomb_scale(n_H, a_set, v_coefs, -1.0/qdiv, v_coefs);
    v_coefs[index] += -1.0/qdiv;
    return 0;
}
...
static inline size_t rank_2_update_removal_index(size_t n_H, size_t n_a, const ordered_set_t* a_set, const indexed_vectors_t *invq_coefs, size_t i, const real_t y[n_H]) {
    ...
    for (size_t j = 0; j < n_a; ++j) {
        ...
        for (size_t n = 0, k = ordered_set_nth(a_set, n); k != ordered_set_end(a_set); k = ordered_set_nth(a_set, ++n)) {
            ...
            real_t invq_kj = lincomb_element(n_H, a_set, invq_coefs_k, &m_m4_cols, j) + (k == j ? 1.0 : 0.0);
...
static inline void update_invq(size_t n_H, size_t index, const ordered_set_t* a_set, const real_t v_coefs[n_H], indexed_vectors_t *invq_coefs) {
    for (size_t n = 0, i = ordered_set_nth(a_set, n); i != ordered_set_end(a_set); i = ordered_set_nth(a_set, ++n)) {
        if (i == index) {
            continue;
        }
        real_t *invq_coefs_i = indexed_vectors_get_mut(invq_coefs, i);
        real_t factor = lincomb_element(n_H, a_set, invq_coefs_i, &m_m4_cols, index); // + (i == index ? 1.0 : 0.0)
        invq_coefs_i[index] = factor*v_coefs[index]; // This is irrelevant if index is leaving, and important if index is joining
        lincomb_add_scaled(n_H, a_set, invq_coefs_i, v_coefs, factor, invq_coefs_i);
    }
}

static void compute_y(size_t n_H, const ordered_set_t* a_set, const real_t y_coefs[n_H], const real_t m[n_H], real_t y[n_H]) {
    lincomb_compute(n_H, a_set, y_coefs, &m_m4_cols, y);
    linalg_vector_add(n_H, y, m, y);
}

static inline void update_y_coefs(size_t n_H, size_t index, const ordered_set_t* a_set, const real_t v_coefs[n_H], real_t y_coefs[n_H], const real_t m[n_H], real_t y[n_H]) {
    real_t yi = y[index]; 
    // real_t yi = lincomb_element(n_H, a_set, y_coefs, &m_m4_cols, index) + m[index];
    lincomb_add_scaled(n_H, a_set, y_coefs, v_coefs, yi, y_coefs);
    y_coefs[index] = yi*v_coefs[index]; // Irrelevant if removal, important if insertion
}

static int active_set_remove(size_t n_H, size_t index, ordered_set_t *a_set, indexed_vectors_t *invq_coefs, const real_t m[n_H], real_t y[n_H]) {
    int err = compute_v(n_H, a_set, invq_coefs, index, 1.0, m_v_coefs); // OBS! Krever gammel invq_coefs/invq, mens v_i krever ny active set og kolonne
    if (err) {
        return err;
    }
    update_y_coefs(n_H, index, a_set, m_v_coefs, m_y_coefs, m, y);
    update_invq(n_H, index, a_set, m_v_coefs, invq_coefs);
    indexed_vectors_remove(invq_coefs, index);
    indexed_vectors_remove(&m_m4_cols, index);
    ordered_set_remove(a_set, index);
    compute_y(n_H, a_set, m_y_coefs, m, y);
    return 0;
}

static int active_set_insert(size_t n_H, size_t index, ordered_set_t *a_set, indexed_vectors_t *invq_coefs, const real_t m[n_H], real_t y[n_H]) {
    int err = compute_v(n_H, a_set, invq_coefs, index, -1.0, m_v_coefs);
    if (err) {
        return err;
    }
    update_invq(n_H, index, a_set, m_v_coefs, invq_coefs);
    indexed_vectors_insert(invq_coefs, index); // Create slot so memory can be manipulated
    memcpy(indexed_vectors_get_mut(invq_coefs, index), m_v_coefs, sizeof(real_t)*n_H);
    update_y_coefs(n_H, index, a_set, m_v_coefs, m_y_coefs, m, y);
...
static int algorithm1(size_t n_H, size_t n_a, ordered_set_t *a_set, indexed_vectors_t *invq_coefs, const real_t m[n_H], real_t y[n_H]) {
    while (1) {
        ...
        if (index != n_H) {
            if (index < n_a) {
                int err = active_set_insert(n_H, index, a_set, invq_coefs, m, y);
                if (err) {
                    return err;
                }
            } else {
                int err = active_set_remove(n_H, index, a_set, invq_coefs, m, y);
                if (err) {
                    return err;
                }
            }
            ...
            if (active_constraints(a_set, n_a) == n_a) {
                ...
                if (index2 == n_H) {
                    ...
                    int err = active_set_insert(n_H, index2, a_set, invq_coefs, m, y);
                    if (err) {
                        return err;
                    }
                } else {
                    int err = active_set_remove(n_H, index2, a_set, invq_coefs, m, y);
                    if (err) {
                        return err;
                    }
                }
...
int ramp_solve(size_t n_H, size_t n_a, int hotstart_variant, const real_t m[n_H], real_t y[n_H]) {
    ...
    compute_y(n_H, &m_a_set, m_y_coefs, m, y);
